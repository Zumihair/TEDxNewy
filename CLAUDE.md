...
Two Claude Code skills live at `.claude/skills/tedxnewy-design/` (the public
site) and `.claude/skills/tedxnewy-admin-design/` (the `/admin` back office).
Each was generated by reading this repo (`app/globals.css`, the component
inventory, `app/admin/*`) plus the brand guides in the workspace `Assets/`
library, and packages tokens, component references, content voice rules and
click-through UI kits for its surface. They're versioned here (the
`.gitignore` blanket-ignores `.claude/` except `.claude/skills/`) so any
session on any machine gets them after a pull, and Claude Code auto-loads
them as invocable skills for work under this repo.

Reach for one of these before hand-rolling a colour, spacing value, radius or
component shape: `tedxnewy-design/readme.md` and `tedxnewy-admin-design/readme.md`
are the fast way to become fluent in the respective system without re-deriving
it from `globals.css`. Each `components/**/*.prompt.md` names what a component
is for and when to reach for it.

**Asset duplication (fixed 2026-08-25).** Each skill's `assets/` folder used
to be a copy of files that already live in `public/` (fonts, logos,
event/speaker photography) — necessary while these were standalone packages,
pure drift risk once they moved into this repo. Both `assets/` folders are
now deleted; every guideline, component preview, UI kit and prompt doc links
straight to the real `public/` files instead (verified by resolving every
rewritten relative path against disk). The compiled `_ds_bundle.js` and
`_ds_manifest.json` in each skill still carry the old `assets/...` paths —
they're generated by Claude Design's own tooling, not hand-maintained, and
only matter again if this project is pushed back to the claude.ai Design
System pane via `/design-sync` (see each skill's `github.md` for the caveat).
The admin bundle is also behind the toaster below: it exports no `Toast`, and
its `Flash` still carries the `ok` tone the real one dropped. The hand-written
files (`components/`, `ui_kits/`) are current; only the compiled pair is not.

**Token accuracy (verified 2026-08-25).** Every value in both skills'
`tokens/*.css` was grepped against the live `app/`, `components/`, `lib/`
trees rather than taken on trust. The admin skill's tokens are 100% real
(some as Tailwind default utilities rather than named constants — e.g.
`shadow-modal` is Tailwind's built-in `shadow-2xl`). The site skill's tokens
checked out real with two exceptions, marked `@status: proposed` inline:
`--cream-bar` and `--shadow-panel-dark` don't exist verbatim anywhere in the
codebase yet. This did **not** mean touching the live app: `app/globals.css`'s
`@theme` block and `app/admin/section-theme.ts` are already each a genuine
single source for what they define: the "duplication" is that a lot of the
sitewide colour vocabulary only exists as repeated hex literals, never
promoted to a named token, which is what the skill's fuller token layer is
proposing. Turning that into one real, no-drift source (promoting those
literals into `@theme`, replacing the hex in `globals.css`'s `.btn-*`/`.pill-*`
classes and across `.tsx` files with `var()` references) is real production
work with broad blast radius, not yet started — see the design-system
...
- **Admin performance patterns:** auth is deduplicated per request via React
  ...
  `window.confirm`/`window.prompt`: use `app/admin/ConfirmDialog.tsx`. No
  inline banner for the result of an action either: that is the toaster
  above (`useToast()` / `<FlashToast>`). `Flash` has no `ok` tone left, so
  a green "Saved" banner will not compile.
