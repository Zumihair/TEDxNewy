...
export default async function MediaPage({
  ...
  const tab: Tab = tabParam === "release" ? "release" : "contacts";
  ...
  const release = signalMediaRelease(sender);
  const pitchBody = defaultPitchIntro(null, sender) + release;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <TabChip href="/admin/media?tab=contacts" active={tab === "contacts"}>
          Contacts · {contacts.length}
        </TabChip>
        <TabChip href="/admin/media?tab=release" active={tab === "release"}>
          New release
        </TabChip>
      </div>
          ...
          <Card className="p-6">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11.5px] text-[#8a8278]">
                Sets the signature below. Switching re-writes the draft, so make the switch before editing.
              </span>
            </div>
                    ...
                    {contacts.map((c) => (
                      ...
                      >
                        <td className="py-2.5 pr-4">
                          ...
                          {!c.email && (
                            <div className="text-[12px] text-[#8a8278]">no email yet</div>
                          )}
                        </td>
...
function TabChip({
  ...
  return (
    <Link
      href={href}
      className={
        "rounded-full px-4 py-1.5 font-mono text-[10.5px] font-semibold uppercase transition-all " +
        (active
          ? "bg-[#141210] text-[#f4efe6]"
          : "border border-[rgba(20,18,16,0.12)] bg-white text-[#6b6459] hover:text-[#141210]")
      }
