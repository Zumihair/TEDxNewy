...
| `HUMANITIX_API_KEY` | For ticket sales | Read-only Humanitix API key behind `/admin/tickets` (live sales, attendee import) and the Monday ticket digest. Leave unset and both show a "not configured" notice rather than erroring — nothing else on the site reads ticket data |
