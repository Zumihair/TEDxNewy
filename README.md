...
| Socials (`/admin/socials`) | `social_posts`/`social_post_media` — the drafts log for Instagram, Facebook and LinkedIn. Write the caption (with optional per-channel versions and live character counts), design carousel graphics in the embedded Creative studio (same tool as `/team-brand?view=creative`; designs save their spec so they stay editable) or upload finished images, and move each post Draft → Changes needed → Ready to post → Posted. **Publishing goes through Buffer** (`lib/buffer-social.ts`, `social_connections`): channels are connected in Buffer's own dashboard, then **Sync from Buffer** on the Connections card reads that list back. A synced channel gets a real Publish button (phone-mockup preview, confirm, posts immediately) on Ready-to-post; an unconnected channel keeps the original manual run sheet (copy caption, download graphics, open channel, mark as posted) |
