...
| `/signal/links` | Event-day resource hub for Signal, reached only by scanning a QR code during event week (19 to 25 October), NOT linked from anywhere on-site. A standalone microsite look distinct from the rest of the site (no Nav/Footer, its own dark palette, the "Authenticity" 2026 event artwork as a backdrop): fades in on an Acknowledgement of Country screen, then (Continue) a 3x2 tile grid (Program, Speakers, Event Week Guide, Sponsors, About TEDxNewy, Signal Activity). Every tile opens a shared near-fullscreen modal (`TileModal`) rather than navigating away or opening a file, so nobody loses their place mid-event. **Two compositions, split at `md` (768px).** Below that it is the phone hub: a 420px column and a 2x3 grid of icon-and-label tiles, all on one screen with nothing to scroll. From `md` up it is a different layout rather than a wider version of the same one, because the phone column centred on a desktop read as a tall narrow strip in empty space. The identity sits in a horizontal band across the top and the tiles become content cards, two across on a tablet and three from `lg`, each previewing a real slice of its own modal behind a "See more" pill. Every preview is derived from the same data the modal renders (`SIGNAL_AGENDA`, the CMS speakers and sponsors, `GUIDE_SUMMARY` computed from the guide's own pages, the About stats), so a preview cannot drift from what opens when you tap it; Signal Activity has no content yet and says so rather than faking one. The two subtrees are deliberately separate (`md:hidden` and `hidden md:flex`) rather than one tree bent with responsive utilities, so the signed-off phone layout is untouched and costs no layout or paint on a phone. The preview images are deliberately NOT lazy: the page already warms every speaker and sponsor image at every width, so lazy saved a phone nothing and only delayed thumbnails that sit at the top of a desktop viewport (measured on production: still blank at 1.2s, filled by 3s). A modal grows out of the tile that was tapped and shrinks back into it: the tile's centre becomes the panel's `transform-origin` and the panel scales uniformly from 0.34, so it stays compositor-only and the header text never squashes. The scrim fades ACROSS that growth rather than ahead of it, and that is the whole thing that makes it read: sequenced ahead, as it was briefly, the scrim hit full opacity by 125ms and hid the tile before the panel had moved, leaving nothing to grow out of. The Acknowledgement of Country fades in as ONE block, with no per-element stagger and no child that can reveal itself separately, because a staggered version read as the page still loading line by line. It is a transition started a frame after mount rather than a CSS animation, since an animation runs from first paint, which on a phone opening this from a QR scan is early enough that the fade is over before the screen is looked at. **Reduce Motion is a designed path here.** The blanket reduced-motion rule in `globals.css` collapses transition DURATION but leaves transition DELAY standing, so any staged reveal becomes hard snaps on the original schedule; `.rm-fade` / `.rm-fade-slow` opt this page's key elements back into a plain cross-fade, and `prefersReducedMotion()` in `TileModal.tsx` drops the scale entirely. Program and Sponsors reuse `SIGNAL_AGENDA`/`SIGNAL_SPONSOR_EXCLUDE`/`SIGNAL_LOGO_SCALE` from `lib/signal-content.ts`, the same constants `/signal` itself uses. Speakers pulls the same CMS call `/signal` uses (`getSpeakersWithTalksForEvent`) but renders its own compact photo-grid, not `/signal`'s `SpeakerLineup`/`SignalSpeakerCard`/`SignalSpeakerPuzzle`; tapping a speaker swaps the SAME modal's content for an inline bio (back, plus prev/next), rather than navigating to `/signal?speaker=<slug>` or stacking a second modal on top. That grid is 2 columns by 3 rows and takes its height from the panel (`grid-rows-3`, each cell `min-h-0`, the name overlaid on the photo), so the whole lineup lands on one screen with nothing to scroll whatever the phone. Sponsors is the one tile whose modal sizes to its own content (`fit` on `TileModal`) instead of filling the screen. About TEDxNewy carries two stats and two small buttons out to `/events` and `/talks`. There is no site address at the bottom of the hub: the TEDxNewy logo at the top is the link home. The Event Week Guide is real HTML (`GuideGallery.tsx`) as of 2026-09-23, shown as an in-modal swipeable gallery rather than a PDF download or viewer; only the venue map page is still an image. `noindex`, excluded from `sitemap.xml`, listed in `robots.txt`'s disallow. **It still has its own share card**, and those two facts are unrelated: robots directives govern search indexing, not the link unfurlers in iMessage, WhatsApp, Slack and the socials. Without `app/signal/links/opengraph-image.tsx` the route inherited `/signal`'s card (Next resolves that file from the nearest ancestor SEGMENT), so a shared guide previewed as a performance photo from a previous year captioned "Signature · Signal.". Its own card is built on the same "Authenticity" artwork the page uses as a backdrop, reads "Signal · 24 October / Your event guide.", and is the one card that sets `monoLogo` (the artwork is a near-saturated red field, so the two-tone wordmark's red half all but disappears on it). The page declares no `openGraph` object on purpose: Next replaces rather than merges that object across segments, so declaring one drops the root layout's `og:site_name` and `og:locale` |
